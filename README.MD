...
运行全链路 Smoke Test（需要服务已启动）：

```bash
bash hack/smoke-test.sh
```

脚本会依次验证注册、验证码、登录、用户资料、团队、成员、心跳、任务创建/编辑/状态流转/详情/热门、评论、点赞、提醒、通知列表/未读数/已读、动态流和登出黑名单，共覆盖全部 28 个 API 端点。
...
| 模块 | 状态 |
| --- | --- |
| 用户注册/登录/验证码/JWT/退出黑名单 | ✅ |
| 用户资料 Cache Aside | ✅ |
| 团队创建/成员管理/成员缓存 | ✅ |
| 在线状态心跳/团队在线成员 | ✅ |
| 任务创建/列表/详情/编辑/状态流转 | ✅ |
| 任务详情缓存（穿透/失效/TTL 抖动/击穿互斥重建） | ✅ |
| 通知中心（列表/未读数/已读/指派/状态/评论/提及/提醒） | ✅ |
| 团队动态流 | ✅ |
| 热门任务排行 | ✅ |
| 创建任务限流/登录限流 | ✅ |
| 分布式锁工具 + Lua 原子释放 | ✅ |
| 工单评论 + 提及通知 + 负责人评论通知 | ✅ |
| 延迟提醒（ZSet 队列/到期扫描/scanner 分布式锁） | ✅ |
| 点赞/取消点赞/状态查询 | ✅ |
| 通知重试队列（List 入队/出队/worker/worker 分布式锁） | ✅ |
| 前端控制台（看板/评论/点赞/提醒/通知/动态/调试面板） | ✅ |
| 全链路 Smoke Test | ✅ |
